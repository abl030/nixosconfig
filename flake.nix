        ...
        checks.x86_64-linux = let
          ...
          mongodb80UpdaterCheck =
            pkgs.runCommand "mongodb80-updater-package" {
              ...
              set -euo pipefail
              MONGODB80_PACKAGE_SOURCE=${./nix/pkgs/mongodb80.nix} \
              ...
              MONGODB80_TEST_CANDIDATE_VERSION=${mongodb80CandidateVersion} \
                ${pkgs.python3}/bin/python3 ${./nix/checks/test_mongodb80_update.py}
              ROLLING_UPDATE_SOURCE=${./scripts/rolling_flake_update.sh} \
                ${pkgs.python3}/bin/python3 ${./nix/checks/test_rolling_mongodb80_transaction.py}
              touch "$out"
            '';
