...
- **Dependencies:** U8.
- **Files:** `scripts/forgejo-auth.sh` (single trace-safe Git/REST boundary), `scripts/rolling_flake_update.sh` (helper + remote-SHA verification), `modules/nixos/ci/rolling-flake-update.nix` (immutable helper packaging and sops token path), `hosts/proxmox-vm/configuration.nix`, and the documented agent/runbook consumers.
- **Approach:** The checked-in boundary validates the exact credential-free Forgejo fetch and push URL sets before reading the token file, disables shell/Git/curl tracing for the short-lived child, and never places credentials in URLs or argv. Dev boxes hold no Forgejo push token; reviewed relays run through doc1. The nixbot token remains doc1-owned and repo-scoped, while the separate all-scope Hermes token remains the REST control-plane credential. `populate_cache.sh`/`hash-capture.sh` build from the local checkout — unaffected.
- **Test scenarios:** `ONLY_GROUP=llm` run end-to-end: clone from Forgejo, signed commit, push through the boundary, mirror propagates to GitHub; bot token absent/empty/unreadable → push fails before authenticated network and the night is flagged; exact wrong or multiple fetch/push URLs are rejected before token read; process arguments, journald, failure artifacts, and triage logs never contain the Forgejo token; a doc1 token revoke-and-reissue tabletop proves operator recovery; personal GitHub direct push to master is rejected.
