...
- **Push-mirror Forgejo→GitHub via API** (per-repo DB state, `sync_on_commit: true`), authenticated by a fine-grained GitHub PAT with Contents:RW on this repo only. If mirroring workflow files ever requires Workflows:RW, that is a blocker and needs a new scope decision rather than silent widening. Mirror health is actively monitored by polling `last_error` (Forgejo surfaces mirror failures nowhere else) with a dedicated read-only Forgejo poller token. **GitHub master gets a repository ruleset / protected-branch control with a concrete bypass actor model** (scope amendment approved 2026-06-10): direct updates/deletions are restricted to the dedicated GitHub mirror machine user `abl030-forgejo-mirror`, not an ambiguous credential.
...
### U8. Forge prep: anonymous read, repo, machine account, push-mirror, GitHub lock
...
- **Files:** `modules/nixos/services/forgejo.nix` (`REQUIRE_SIGNIN_VIEW = false`; `[mirror]` settings), `secrets/hosts/proxmox-vm/forgejo-nixbot-token` (new doc1/proxmox-vm-scoped bot push token), `secrets/hosts/doc2/forgejo-github-mirror-pat` (new doc2-only GitHub mirror PAT), `secrets/hosts/doc2/forgejo-mirror-poller-token` (new doc2-only read-only Forgejo API token), `modules/nixos/services/alerting.nix` or monitoring wiring for the mirror poller, `docs/wiki/services/forgejo.md`.
- **Approach:** Flip `REQUIRE_SIGNIN_VIEW` off (other repos stay private per-repo — verified semantics: anonymous smart-HTTP/archive for public repos only). Push full history from doc1; repo explicitly public. Create dedicated Forgejo accounts/tokens for the write path: `nixbot` for the nightly bot plus per-machine writer accounts for doc1, epimetheus, framework, and wsl, grouped in a dedicated `nixosconfig-writers` team. Personal accounts are not `master` writers. Create `nixbot` (collaborator write on nixosconfig only) + v15 repo-scoped `write:repository` token → `secrets/hosts/proxmox-vm/forgejo-nixbot-token`, owned/readable by the `rolling-flake-update.service` user, not root-only. Configure Forgejo branch protection for `master`: only `nixosconfig-writers` can push ordinary updates, owner/admin bypass is disabled, and force-push/delete are disabled for everyone. Configure push-mirror via API (`sync_on_commit: true`, interval as belt) with a fine-grained GitHub PAT from `secrets/hosts/doc2/forgejo-github-mirror-pat`, decryptable on doc2 only and scoped Contents:RW on this repo only; workflow-file mirror failure is a blocker, not permission-widening. Document that Forgejo stores the mirror credential in repo DB state and that dumps/backups containing that state are credential-bearing secret material: encrypted at rest, restricted read/restore access, explicit retention/deletion rules, and mandatory mirror PAT rotation after any restore, dump extraction, or off-host copy before mirror sync is re-enabled. Verify logs, API errors, `last_error`, and dumps do not expose the token in plaintext. **Flip order matters**: freeze pushes → final sync → enable mirror → U8 synthetic propagation test → U9 bot/dev repoint — never both bot-push-to-GitHub and mirror-force-push live simultaneously. Add the approved GitHub repository ruleset / protected-branch control for `master` with `abl030-forgejo-mirror` as the sole update/delete bypass principal. Mirror health: small timer on doc2 polling `GET /api/v1/repos/abl030/nixosconfig/push_mirrors` using `secrets/hosts/doc2/forgejo-mirror-poller-token`, a dedicated minimum-scope read token; explicitly forbid reusing the GitHub mirror PAT, nixbot token, or an admin token for the poller. Poller contract: exactly one push mirror must exist for the expected GitHub target URL; empty, multiple, wrong-target, null `last_update`, non-empty `last_error`, or stale `last_update` all log the alert marker. PAT expiry/rotation is the expected rot vector — alert and runbook cover it.
- **Test scenarios:** Anonymous `git clone https://git.ablz.au/abl030/nixosconfig` from a fleet host succeeds; anonymous access to a private repo (agents) still 404s; nixbot token pushes to nixosconfig but cannot touch other repos or repo settings; dedicated per-machine Forgejo writer accounts can push ordinary signed updates, while personal accounts cannot push ordinary commits, force-push, or delete `master`; a synthetic signed test commit pushed to Forgejo appears on GitHub within the sync window; a direct push to GitHub master with a personal token is rejected by the GitHub ruleset/protection and only `abl030-forgejo-mirror` can update `master`; poller fires on missing/multiple/wrong-target/stale/error mirror rows and on a deliberately broken mirror credential; mirror PAT rotation replaces the credential and revokes the old PAT; restore/dump extraction tabletop rotates the mirror PAT before re-enabling sync; negative log-redaction tests prove no token appears in API errors, `last_error`, journald, or failure artifacts.
- **Verification:** Mirror setup passes the synthetic propagation test and protection tests; the 48h real-bot mirror soak happens after U9.
...
- **Requirements:** R7, R14, R15; origin Scope Boundaries (issues stay on GitHub — documented).
- **Dependencies:** U10.
...
- Origin R12 put the host-origin canary before the bot/fleet cutover. This plan amends R12 deliberately: the Forgejo write path moves first (U9) while hosts still fetch GitHub, then a 48h real-bot mirror soak proves Forgejo→GitHub propagation, then igpu canaries Forgejo as a host fetch origin (U10), then the fleet flips. The altered order exists so the canary fetches a live, current Forgejo repo rather than a stale copy.
