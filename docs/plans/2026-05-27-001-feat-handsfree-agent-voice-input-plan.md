...
> **State as of 2026-09-10.** Implemented as a working MVP on the unmerged branch
> `feat/handsfree-agent-voice-input` (commit `dc970b6f`), then parked. Only this
> plan and the research writeup are on master. Findings, the negative result on
> Bluetooth headset buttons, and the two least-privilege items still blocking a
> merge are in
> [docs/wiki/claude-code/handsfree-android-voice-input.md](../wiki/claude-code/handsfree-android-voice-input.md).
