...
**Status:** ✅ stable as of **2026-09-24**. This is the canonical record of prom's kernel flags, boot pool, bootloader setup, and cluster/quorum posture.
...
## LAN-boundary ARP capture (retired 2026-09-24)

Prom ran a bounded two-direction `tcpdump` ARP capture on `enp8s0`
(`prom-arp-capture@{in,out}.service`) from 2026-09-22 to 2026-09-24 for issue
#223. It caught the recurrence that identified the cause: the USW Flex Mini
transiently claims Ubiquiti's factory fallback address `192.168.1.20`. The
services, wrapper, unit file and pcaps were then removed from Prom. Findings and
fix: [unifi-fallback-ip-conflict.md](unifi-fallback-ip-conflict.md). The scripts
can be recovered from commits `73cd43be` and `e9b7c7b2` if a future L2 fault
needs the same capture.
