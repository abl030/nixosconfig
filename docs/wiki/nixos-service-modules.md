...
2. An ordinary systemd timer `deep-probe-<slug>.timer` (`OnUnitActiveSec = interval`, AccuracySec 1s) that preserves the probe's normal cadence.
3. A distinct calendar timer/service pair `post-maintenance-deep-probe-<slug>.{timer,service}` at 05:31 Australia/Perth. Both services execute the same immutable probe runner under the same per-probe runtime-directory lock and runtime user. The calendar path waits up to eight minutes for an overlapping ordinary invocation, then runs the probe itself while still holding the lock. Ordinary probes have an explicit 300s maximum start timeout, 90s stop timeout, and control-group kill policy, so the lock wait retains 90s of margin even when a TERM-ignoring descendant inherits the lock descriptor. If the ordinary timer fires while the calendar path owns the lock, it exits successfully as a duplicate. The calendar service has a 15-minute timeout, covering both the bounded wait and the longest generated probe timeout plus heartbeat retry overhead.
4. A systemd oneshot `deep-probe-<slug>.service` (TimeoutStartSec `timeout`). The oneshot runs the probe command; on exit 0 it curls the push URL with `status=up&ping=<ms>`. On any non-zero exit OR timeout it does NOT push — Kuma misses heartbeats and flips DOWN after `maxretries` (default 10).

The separate post-maintenance target is required by systemd timer semantics: if a
timer elapses while its target service is already active, systemd leaves that
service running and does not queue another invocation. Combining `OnCalendar`
with the ordinary timer therefore cannot guarantee a fresh post-maintenance run
for probes that may overlap 05:31. The shared lock closes the wait/start race
without interrupting a probe or duplicating its implementation.
...
homelab.tailscaleShare.<name> = {
  ...
  publishIpv6 = true;                             # default; AAAA → tailscale IPv6 avoids sharee IPv4 remapping
