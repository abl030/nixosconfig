...
Doc landed on master: 2026-09-10
...
This page is research plus a live MVP result. **The implementation it describes is
not on master.** It sits on the unmerged branch `feat/handsfree-agent-voice-input`
(commit `dc970b6f`, 2026-05-29), which holds the `talk-to-me` skill changes, the
four `agent-voice-*` scripts, and the sideloaded trigger app under
`tools/agent-voice-trigger-android/`. Read a file from it without checking out:

```bash
git show feat/handsfree-agent-voice-input:.claude/skills/talk-to-me/scripts/agent-voice-input-termux.sh
```

Two things to settle before that branch lands, both flagged in
[Least Privilege Notes](#least-privilege-notes) below and unaddressed in the MVP:

- The phone holds an unrestricted SSH key to doc1 and the inject helper types
  into a live agent pane and presses Enter. That key needs a forced command
  scoped to `agent-voice-inject`, not general shell.
- Setup flips `allow-external-apps = true` in the phone's `termux.properties`,
  which widens what any installed Android app can ask Termux to run.

The transcription leg is unaffected and still in production: `whisper.ablz.au`
(`modules/nixos/services/whisper-server.nix`, running on igpu) serves the Dictate
keyboard today.
