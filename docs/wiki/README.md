...
- [voice-diary](services/voice-diary.md) — car voice notes → dated transcript pairs: phone/Syncthing/timer pipeline on doc2, why a timer and not a `systemd.path` unit on NFS, why the drop dir is read-only (the phone holds the only other copy), why no AI is in the loop; **tuning transcript quality** (name fixes, vocabulary prompt, paragraph shaping) plus measured findings — what prompting can't fix, why beam search was rejected, and how to measure a recording before blaming the decoder
