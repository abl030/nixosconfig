...
"""Rebuild Famous Five chapter boundaries from FadedPage TOCs and spoken headings."""
...
ROOT = Path("/mnt/data/Media/Books/Audiobooks/Enid Blyton/Famous Five")
...
class Book:
    ...
    @property
    def fadedpage_title(self) -> str:
        return self.search_title or self.title


BOOKS = {
    1: Book(1, "Five on a Treasure Island", "1 - Five on a Treasure Island", "1 - Five on a Treasure Island.m4b"),
    2: Book(2, "Five Go Adventuring Again", "2 - Five Go Adventuring Again", "2 - Five Go Adventuring Again.m4b"),
    3: Book(3, "Five Run Away Together", "3 - Five Run Away Together", "3 - Five Run Away Together.m4b"),
    4: Book(4, "Five Go to Smuggler's Top", "4 - Five Go to Smugglers Top", "4 - Five Go to Smugglers Top.m4b"),
    5: Book(5, "Five Go Off in a Caravan", "5 - Five Go Off in a Caravan", "5 - Five Go Off in a Caravan.m4b"),
    6: Book(6, "Five on Kirrin Island Again", "6 - Five on Kirrin Island Again", "6 - Five on Kirrin Island Again.m4b"),
    7: Book(7, "Five Go Off to Camp", "7 - Five Go Off to Camp", "7 - Five Go Off to Camp.m4b"),
    8: Book(8, "Five Get into Trouble", "8 - Five Get into Trouble", "8 - Five Get into Trouble.m4b"),
    9: Book(9, "Five Fall into Adventure", "9 - Five Fall into Adventure", "9 - Five Fall into Adventure.m4b"),
    10: Book(10, "Five on a Hike Together", "10 - Five on a Hike Together", "10 - Five on a Hike Together.m4b"),
    11: Book(11, "Five Have a Wonderful Time", "11 - Five Have a Wonderful Time", "11 - Five Have a Wonderful Time.m4b"),
    12: Book(12, "Five Go Down to the Sea", "12 - Five Go Down to the Sea", "12 - Five Go Down to the Sea.m4b"),
    13: Book(13, "Five Go to Mystery Moor", "13 - Five Go to Mystery Moor", "13 - Five Go to Mystery Moor.m4b"),
    14: Book(14, "Five Have Plenty of Fun", "14 - Five Have Plenty of Fun", "14 - Five Have Plenty of Fun.m4b"),
    15: Book(15, "Five on a Secret Trail", "15 - Five on a Secret Trail", "15 - Five on a Secret Trail.m4b"),
    16: Book(16, "Five Go to Billycock Hill", "16 - Five Go to Billycock Hill", "16 - Five Go to Billycock Hill.m4b"),
    17: Book(17, "Five Get into a Fix", "17 - Five Get into a Fix", "17 - Five Get into a Fix.m4b"),
    18: Book(18, "Five on Finniston Farm", "18 - Five on Finniston Farm", "18 - Five on Finniston Farm.m4b"),
    19: Book(19, "Five Go to Demon's Rocks", "19 - Five Go to Demons Rocks", "19 - Five Go to Demons Rocks.m4b"),
    20: Book(20, "Five Have a Mystery to Solve", "20 - Five Have a Mystery to Solve", "20 - Five Have a Mystery to Solve.m4b"),
    21: Book(21, "Five Are Together Again", "21 - Five Are Together Again", "21 - Five Are Together Again.m4b"),
}
...
def title_case(raw: str) -> str:
    ...
    for token in tokens:
        ...
        prev_token = token
    result = "".join(pieces)
    result = re.sub(r"\bTimmy\b", "Timmy", result)
    result = re.sub(r"\bKirrin\b", "Kirrin", result)
    result = re.sub(r"\bBillycock\b", "Billycock", result)
    result = re.sub(r"\bFinniston\b", "Finniston", result)
    result = re.sub(r"\bSmuggler'S\b", "Smuggler's", result)
    result = re.sub(r"\bDemon'S\b", "Demon's", result)
    result = re.sub(r"\bWhat'S\b", "What's", result)
    result = re.sub(r"\bGeorge'S\b", "George's", result)
    result = re.sub(r"\bDick'S\b", "Dick's", result)
    result = re.sub(r"\bJulian'S\b", "Julian's", result)
    result = re.sub(r"\bAnne'S\b", "Anne's", result)
    result = re.sub(r"\bTimmy'S\b", "Timmy's", result)
    return result
...
def parse_toc(text: str) -> list[str]:
    ...
    for line in block.splitlines():
        ...
        raw_title = re.sub(r"\s+", " ", match.group(2).strip()).replace("_", "")
...
def fadedpage_titles(book: Book) -> tuple[str, list[str]]:
    data = fetch_json(
        FADEDPAGE_SEARCH_URL,
        data={
            "title": book.fadedpage_title,
            "author": "Blyton, Enid",
            "plang": "--",
            "category": "--",
            "publisher": "",
...
def analyze_book(book: Book, output_dir: Path, *, minimum_silence: float = 3.5, keep_workdir: bool = False) -> dict:
    pid, titles = fadedpage_titles(book)
    ...
    workdir = Path(tempfile.mkdtemp(prefix=f"famous-five-{book.number:02d}-"))
...
def parse_numbers(values: Iterable[str]) -> list[int]:
    selected: list[int] = []
    for value in values:
        if value == "all":
            return sorted(BOOKS)
...
def main() -> int:
    ...
    parser.add_argument("--output-dir", default="/tmp/famous-five-chapters", help="Directory for JSON reports")
    ...
    for number in parse_numbers(args.books):
        book = BOOKS[number]
        try:
