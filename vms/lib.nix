...
with lib; rec {
  ...
  assertVMOperationAllowed = defs: vmid: operation: let
  ...
  # Returns: integer
  getNextVMID = defs: usedVMIDs: rangeStart: rangeEnd: let
  ...
  in
    if availableVMIDs == []
    then throw "No available VMIDs in range ${toString rangeStart}-${toString rangeEnd}"
    else head availableVMIDs;

  # Suggest next VMID based on allocation strategy
  # Returns: integer
  suggestNextVMID = defs: usedVMIDs: let
    ranges = defs.vmidRanges or {};
    productionRange =
      ranges.production or {
        start = 100;
        end = 199;
      };
  in
  ...
  validateVMDefinition = _name: vm: let
    errors =
  ...
  validateAllManagedVMs = defs: let
    managed = defs.managed or {};
