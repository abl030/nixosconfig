...
metadata:
  hermes:
    ...
    source: /home/abl030/nixosconfig/.claude/agents/homeassistant.md
---
...
- This agent expects the Hermes MCP server/toolset `mcp-homeassistant`.
- Source Claude MCP wrapper: `./scripts/mcp-homeassistant.sh`.
- Start a fresh Hermes session after MCP config changes; in TUI use `/reload-mcp` if available.
- For a narrowly-scoped run, start Hermes with `--toolsets mcp-homeassistant,skills,terminal,file` or delegate with `toolsets=["mcp-homeassistant"]`.
