...
metadata:
  hermes:
    ...
    source: /home/abl030/nixosconfig/.claude/agents/playwright.md
---

# playwright

Migrated from `nixosconfig/.claude/agents/playwright.md` so this homelab agent prompt is tracked in git and usable by Hermes.

Hermes integration notes:
- This agent expects the Hermes MCP server/toolset `mcp-playwright`.
- Source Claude MCP wrapper: `./scripts/mcp-playwright.sh`.
- Start a fresh Hermes session after MCP config changes; in TUI use `/reload-mcp` if available.
- For a narrowly-scoped run, start Hermes with `--toolsets mcp-playwright,skills,terminal,file` or delegate with `toolsets=["mcp-playwright"]`.
