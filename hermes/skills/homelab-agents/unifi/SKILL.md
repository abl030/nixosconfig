...
metadata:
  hermes:
    ...
    source: /home/abl030/nixosconfig/.claude/agents/unifi.md
---
...
- This agent expects the Hermes MCP server/toolset `mcp-unifi`.
- Source Claude MCP wrapper: `./scripts/mcp-unifi.sh`.
- Start a fresh Hermes session after MCP config changes; in TUI use `/reload-mcp` if available.
- For a narrowly-scoped run, start Hermes with `--toolsets mcp-unifi,skills,terminal,file` or delegate with `toolsets=["mcp-unifi"]`.
