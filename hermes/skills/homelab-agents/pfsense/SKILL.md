...
metadata:
  hermes:
    ...
    source: /home/abl030/nixosconfig/.claude/agents/pfsense.md
---
...
- Client-specific content filtering for `s-a55` assigns Unbound tags `no_youtube no_news no_reddit` to `100.102.60.123/32`, `fd7a:115c:a1e0::5301:3c7b/128`, and LAN fallback `192.168.1.38/32`. `no_youtube` zones: `youtube.com`, `youtu.be`, `googlevideo.com`, `ytimg.com`, `youtubei.googleapis.com`, `youtube.googleapis.com`, and `youtube-nocookie.com`. `no_news` zones: `news.google.com`, `news.google.com.au`, `abc.net.au`, and `news.com.au`. `no_reddit` zones: `reddit.com`, `redd.it`, `redditstatic.com`, and `redditmedia.com`. All use `always_nxdomain`. Tailnet split-DNS maps all fifteen zones only to pfSense's Tailscale IP `100.123.61.111`: the second global resolver `192.168.1.1` is reached through tower's subnet route and SNATs away the phone identity. Preserve both global resolvers for other names; do not tag tower's source or remove LAN DNS globally. YouTube/news were applied, API-read-back, and phone/control verified 2026-08-31; Reddit was applied and verified the same way on 2026-09-20. For future split-DNS changes, use the persistent doc1 OAuth environment at `/run/secrets/tailscale-acl/oauth`; it has `policy_file` plus DNS read/write scope, verified by idempotent PATCH/readback. Never print its values, and do not request a temporary DNS credential unless the persistent client is revoked or rejected. Full rationale: `docs/wiki/infrastructure/pfsense-dns-resolver.md`.
