...
metadata:
  hermes:
    ...
    source: /home/abl030/nixosconfig/.claude/agents/tower.md
---

# tower

Migrated from `nixosconfig/.claude/agents/tower.md` so this homelab agent prompt is tracked in git and usable by Hermes.
...
- **NFS exports are GENERATED — never hand-edit `/etc/exports` alone.** Per-share rules live in
  ...
  anything you pipe over. Copy rule syntax from an already-scoped share (`magazines.cfg`).
- **`VMBackups` is deliberately scoped (2026-08-21) — do not widen it back to `*`.** Rule:
  `192.168.1.35` ro + `192.168.1.36` ro (doc2's NICs, kopia-mum) and `192.168.1.25` rw (HAOS
