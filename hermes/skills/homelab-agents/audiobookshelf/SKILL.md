...
metadata:
  hermes:
    ...
    source: /home/abl030/nixosconfig/.claude/agents/audiobookshelf.md
---

# audiobookshelf

Migrated from `nixosconfig/.claude/agents/audiobookshelf.md` so this homelab agent prompt is tracked in git and usable by Hermes.

Hermes integration notes:
- This agent does not use a dedicated MCP server; it relies on normal Hermes terminal/file tools and the doc1 bastion environment.
- For a narrowly-scoped delegated run, use `toolsets=["terminal", "file"]` unless the task needs web/browser/etc.
...
10. **Repair chapters if needed**: if the imported item has generic chapters (`Chapter 1`, `001`, etc.), decide whether to:
   ...
   - **Then look for plain-text/public-domain editions** when the book is old enough that a clean text source exists. In practice, FadedPage is often better than Google Books for classic children's series because the downloaded `.txt` file usually has a readable `Contents` section with chapter number, title, and page number in plain text.
     ...
     Also note that some FadedPage books use an unnumbered `Contents` list with one chapter title per line. That is still a good rename-only source when the chapter count matches the local file.
