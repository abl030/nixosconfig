...
- **Title-actually-PATCHed check.** When the OCR identifies the doc and you decide on a clean title, the `title` field MUST appear in your PATCH body. Reading the OCR and *describing* a new title in your report is not enough — scanner-output filenames (`20260104_132324_brw4cd577318e30_000425`) stay raw unless you actually send the PATCH. Re-fetch any retitled doc and confirm `title` no longer matches the original scanner pattern. **Titles are not optional polish — the user has flagged title hygiene multiple times.** This rule covers ALL filename-style titles, not just `brw4cd5…` scanner outputs:
  - Underscore-mangled filenames: `1_Coronation_Build_6-7-23`, `Complete_with_Docusign_BARRETT-LENNARD`, `Adv_FRCGW_clearance_certificate_716...`, `Please_sign_this_document` — all need rewriting to a human-readable title (`Building Inspection - 1 Coronation Street 6 July 2023`, etc.).
  - Numeric suffixes from re-uploads: `(2)`, ` - 1`, `-2.pdf` — strip them in the new title; if a duplicate cluster exists, flag it for the user instead of silently keeping the suffix.
  - Property-doc filename patterns `<address>_<doc-type>_<date>` are especially common — convert `<doc-type>` to a real noun ("Build" → "Building Inspection", "Pest" → "Pest Inspection").
- **Don't reach for the closest existing correspondent.** If the OCR clearly identifies a bank / supplier / agency that isn't in the correspondent list, leave `correspondent=null` and flag — do NOT pick the nearest letter-match from the existing list. Slip seen 2026-04-30: a Commonwealth Bank balance confirmation got `correspondent=7` (HPMC Construction) — no semantic similarity, just both started with consonants. Same trap with two-letter prefixes ("Bunn..." → "Bayswater Mazda"). The correspondent list is closed-world for matching, but unknown senders go to null, not "the closest one I see".
- **Legal-vendor vs postal-address rule (property transactions).** For ATO clearance certificates, settlement docs, and other legal correspondence tied to a property sale, the **legal vendor name on the doc** tells you which property it concerns — not the postal mailing address. Slip seen 2026-04-30: an ATO Foreign Resident CGW Clearance Certificate listing `Vendor name: ANDREW BARRETT-LENNARD` (personally) was filed under Grevillea (the postal address on the cert) when it actually concerned the Coronation Street sale (Andrew was the personal vendor of Coronation). A second cert listing `Vendor name: ANDY & MEAGAN SBL PTY LTD` was for a company-owned property sale. Always read the **Vendor name** field in OCR — if it's an entity (Pty Ltd / Trust), the doc is about that entity's asset; if it's a personal name, the doc is about that individual's personally-held asset. The address is just where they mail it.
- **Property bundle is all-or-nothing.** When applying a property bundle, set ALL of `correspondent` (if known), `storage_path`, the property `tag`, and the `Property` custom field in the same PATCH. Half-bundles (tag set but no storage_path; or storage_path set but no Property cf) are slips that surface in audits. Slip seen 2026-04-30: doc 381 had `tags=[333]` (Riverslea) but `correspondent=null`, `storage_path=null`, no Property cf — the tag alone doesn't route the doc anywhere.
- **Subdivisions inherit the parent property's bundle.** When a single titled lot is subdivided into multiple new lots and re-numbered onto a different street, the new lots **stay on the parent's storage_path / tag / Property cf**. The street name on the new title doesn't earn its own storage path — that would fragment the bundle and lose the audit trail. Concrete case (see local sidecar for which property): the Coronation lot was subdivided into three; two of the rear blocks were re-numbered onto a different street, but all three route to `Coronation`. Don't create new storage_paths for subdivision children.
- **Empty-signal OCR — don't classify from filename/tags alone.** If the OCR head is dominated by repeated `Docusign Envelope ID: …` lines, repeated timestamps, or otherwise contains no substantive document content, the OCR has effectively *no signal*. **Do NOT fall back to inferring classification from**: (a) the filename (`Complete_with_Docusign_BARRETT-LENNARD…` is just Docusign's auto-name, not a content signal); (b) the *existing* correspondent / tags / storage_path on the doc (these may have been auto-applied by a prior pipeline based on the same useless filename, and propagating them locks in the wrong call); or (c) "the rest of this batch was Riverslea so this probably is too". Slip seen 2026-04-30: doc 351 had `Complete_with_Docusign_BARRETT-LENNARD…` as its filename and arrived with `correspondent=36 (Summit), storage_path=2 (Riverslea), tags=[333]` — but the actual document was a TAL life insurance policy (the OCR was 100% Docusign envelope IDs). The pre-existing tags came from the filename, not from content. Flag empty-OCR docs for the user with "OCR has no usable content; the existing tags came from the filename and may be wrong" — don't classify confidently.
