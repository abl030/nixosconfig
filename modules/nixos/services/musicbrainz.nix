{
  ...
  lrclibPkg = pkgs.rustPlatform.buildRustPackage {
    ...
    cargoLock = {
      lockFile = "${inputs.lrclib-src}/Cargo.lock";
    ...
    if printf '%s' "$out" | ${pkgs.gnugrep}/bin/grep -qE "$failed_re"; then
      ...
      if [ -n "$schema_line" ]; then
        ...
        if [ -n "$codebase" ] && [ -n "$db_seq" ] && [ "$((codebase - db_seq))" = 1 ]; then
          ...
          # UpdateDatabasePrivileges.pl) need.
          #
          # The image's entrypoint dumps the local::lib env (PATH, PERL5LIB,
          # PERL_LOCAL_LIB_ROOT, PERL_MB_OPT, PERL_MM_OPT) to
          # /noninteractive.bash_env; without sourcing that file `carton`
          # itself can't find Carton::CLI. `podman exec` doesn't re-run
