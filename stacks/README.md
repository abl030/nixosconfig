...
Auto-update restarts **user-scope** units named `podman-compose@<project>.service` (generated by `mkService`),
so the user unit reuses the already-decrypted env files under `/run/user/<uid>/secrets` and does **not** run sops.
