...
Stack updates are orchestrated by the rootless `podman-auto-update.service` wrapper, which runs each stack's
`*-update.service` unit. Each update unit executes:

1. `podman compose pull`
2. `podman compose up -d --remove-orphans`

Do not add `io.containers.autoupdate=registry` labels to compose services; Podman compat-path auto-update is not used for compose-managed stacks.
...
**Do not add Podman auto-update labels** to compose services (`io.containers.autoupdate=registry`).
